...
class Motor {
 public:
  ...
  virtual double GetCurEnc() const = 0;

  //@brief Getting Current Encoder value in degrees
  virtual double GetCurEncDeg() const = 0;

  //@brief Getting Current Encoder value in rotations
  virtual double GetCurEncRot() const = 0;
  ...
  virtual void MoveIncEncCounts(double speed, double counts,
                                bool wait = false) = 0;
  ...
  //@param wait when is true, function wait flag 'IsReady'
  virtual void MoveIncDeg(double speed, double deg, bool wait = false) = 0;

  //@brief Move motor from GetCurEncRot to GetCurEncRot+'rots' (increment)
  //@param speed measured in deg/sec
  //@param rots measured in rotations
  //@param wait when is true, function wait flag 'IsReady'
  virtual void MoveIncRot(double speed, double rots, bool wait = false) = 0;

  //@brief Move motor from 'CurEnc' to 'counts' (set)
  //@param speed measured in deg/sec
  //@param counts measured in EncCounts
  //@param wait when is true, function wait flag 'IsReady'
  virtual void MoveToEncCounts(double speed, double counts, bool wait = false) = 0;

  //@brief Move motor from 'CurEnc' to 'deg' (set)
  //@param speed measured in deg/sec
  //@param deg measured in degrees
  //@param wait when is true, function wait flag 'IsReady'
  virtual void MoveToDeg(double speed, double deg, bool wait = false) = 0;

  //@brief Move motor from 'CurEnc' to 'rots' (set)
  //@param speed measured in deg/sec
  //@param rots measured in degrees
  //@param wait when is true, function wait flag 'IsReady'
  virtual void MoveToRot(double speed, double rots, bool wait = false) = 0;
