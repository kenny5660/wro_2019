 ...
 */
void I2c_init(MyRio_I2c* i2c) {
	uint8_t selectReg;
	if (i2c->addr == I2CAADDR) {
		NiFpga_ReadU8(myrio_session, SYSSELECTA, &selectReg);
		selectReg = selectReg | (1 << 7);
		NiFpga_WriteU8(myrio_session, SYSSELECTA, selectReg);
	}
	if (i2c->addr == I2CBADDR) {
		NiFpga_ReadU8(myrio_session, SYSSELECTB, &selectReg);
		selectReg = selectReg | (1 << 7);
		NiFpga_WriteU8(myrio_session, SYSSELECTB, selectReg);
	}
	I2c_Counter(i2c, 255);
	I2c_Configure(i2c, I2c_Enabled);

}
void I2c_start(MyRio_I2c* i2c,int port) {
