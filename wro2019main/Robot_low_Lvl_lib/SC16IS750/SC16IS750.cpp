...
SC16IS750::SC16IS750(std::shared_ptr<Spi> spi, std::shared_ptr<GPIO> ss_pin):
	spi_(spi),ss_pin_(ss_pin),peek_flag(0)
{
	ss_pin_->Set();
...
void SC16IS750::pinMode(uint8_t pin, PinMode pin_mode)
{
	GPIOSetPinMode(pin, pin_mode);
}
...
uint8_t SC16IS750::ReadRegister(uint8_t reg_addr)
{
	uint8_t result = 0;
	ss_pin_->Reset();
	spi_->Transmit(0x80 | (reg_addr << 3));
	result = spi_->Transmit(0xFF);
	ss_pin_->Set();
...
uint8_t result = 0;
	ss_pin_->Reset();
	spi_->Transmit((reg_addr << 3));
	result = spi_->Transmit(val);
	ss_pin_->Set();
...
void SC16IS750::GPIOSetPinMode(uint8_t pin_number, PinMode pin_mode)
{
    uint8_t temp_iodir;
