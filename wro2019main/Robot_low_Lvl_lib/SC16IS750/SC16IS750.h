...
{ 
public:
	SC16IS750(std::shared_ptr<Spi> spi, std::shared_ptr<GPIO> ss_pin);
	enum class PinMode
	{
		OUTPUT = 0,
		INPUT  = 1
	...
	int available();
	void pinMode(uint8_t pin, PinMode pin_mode);
...
private:
	...
	void    GPIOSetPinMode(uint8_t pin_number, PinMode pin_mode);
