...
void simulationStepStarted_CB(std::vector<msgpack::object>* msg) {
  ...
  if (it != data.end()) simTime = it->second.as<float>();
  std::cout << "Simulation step started. Simulation time: " << simTime
            << std::endl;
}
...
void simulationStepDone_CB(std::vector<msgpack::object>* msg) {
  ...
  std::map<std::string, msgpack::object>::iterator it =
      data.find("simulationTime");
  if (it != data.end()) simTime = it->second.as<float>();
  std::cout << "Simulation step done. Simulation time: " << simTime
            << std::endl;
  doNextStep = true;
}
...
void image_CB(std::vector<msgpack::object>* msg) {
  ...
  cl->simxSetVisionSensorImage(sens2, false, img.c_str(), img.size(),
                               cl->simxDefaultPublisher());
}
...
int main(int argc, char* argv[]) {
  ...
  client.simxGetVisionSensorImage(sens1, false,
                                  client.simxDefaultSubscriber(image_CB));
